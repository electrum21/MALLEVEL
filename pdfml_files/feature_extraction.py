...
def feature_extraction(filepath):
    ...
    command_to_execute_Javier='C:\\Python38\\python.exe "' + os.path.join(LOKI_PATH,"pdfid.py") + '" "'+filepath+'"'
    
    stdout = Popen(command_to_execute, shell=True, stdout=PIPE).stdout
    
    output = stdout.readlines()
    #print(command_to_execute_Javier)
    if len(output) == 24:
...
# name=os.path.join(LOKI_PATH, 'CSIT Internship Guide.pdf')
