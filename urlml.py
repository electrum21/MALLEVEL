from constants import IOCS_DIR_PATH, URLML_MODEL_PATH
import re, os, pickle
import warnings
import base64
warnings.filterwarnings('ignore')

def url_blacklist_lookup(url, url_blacklist_dict):
    url = str(base64.b64decode(url))[2:-1]
    if url in url_blacklist_dict:
      reason = url_blacklist_dict[url]
      return True, reason
    return False, None
...
for x in os.listdir(URLML_MODEL_PATH):
    clf_pickle = open(URLML_MODEL_PATH+'/'+x, 'rb')
